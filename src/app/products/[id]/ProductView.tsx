...
export default function ProductView({
  ...
  const gallery = useMemo(() => {
    ...
    return product.image
      ? [{ src: product.image, primary: true }]
      : [];
        ...
        <div className="mt-6 grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          <div>
            <Card className="overflow-hidden">
              <div className="relative bg-gradient-to-br from-slate-50 via-white to-ae-blue/5 px-4 py-5 sm:px-6 sm:py-6">
                ...
                <div className="relative mx-auto flex max-w-3xl flex-col">
                  <div className="relative mx-auto flex h-[300px] w-full max-w-[540px] items-center justify-center overflow-hidden rounded-[28px] border border-black/10 bg-white shadow-[0_18px_50px_rgba(2,6,23,0.08)] sm:h-[360px]">
                    {activeImage ? (
                      <Image
                        src={activeImage}
                        alt={product.name}
                    ...
                    {gallery.length > 1 ? (
                      <>
                        <button
                          type="button"
                          onClick={showPrev}
                          className="absolute left-3 top-1/2 inline-flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-black/10 bg-white/90 text-ae-black shadow-sm transition hover:bg-white"
                        ...
                        <button
                          type="button"
                          onClick={showNext}
                          className="absolute right-3 top-1/2 inline-flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-black/10 bg-white/90 text-ae-black shadow-sm transition hover:bg-white"
